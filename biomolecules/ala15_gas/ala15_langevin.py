...
mol = read(f"{sol}.pdb")
print("Loaded structure from:", f"{sol}.pdb")
print(f"Mol has {len(mol)} atoms.")
...
dyn = Langevin(
    ...
    friction=0.01 / units.fs,  # friction coefficient (typically 0.001-0.01 /fs)
    logfile=f"{sol}_dyn.log",
    trajectory=f"{sol}_dyn.traj",
    loginterval=log_interval,
)

dens_csv = open(f"{sol}_density.csv", "w", buffering=1)
